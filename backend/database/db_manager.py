...
Адаптировано для веб-приложения
"""
import sqlite3
import json
...
class DatabaseManager:
    ...
    def __init__(self, db_path: str = None):
        ...
        Args:
            db_path: Путь к файлу базы данных (если None, используется путь на сервере)
        """
        if db_path is None:
            # Используем путь относительно backend директории
            base_dir = Path(__file__).parent.parent
            self.db_path = base_dir / "data" / "history.db"
        else:
    ...
    def add_generation(self, gen_type: str, prompt: str, model: str, 
        ...
        Args:
            ...
            image_path: Путь к сохраненному изображению (относительный путь на сервере)
